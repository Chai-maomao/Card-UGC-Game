...
func _process(delta: float) -> void:
	_elapsed += delta
	if _role == 2 and _room_ready_count >= 1 and not _disconnect_started and _elapsed >= 4.0:
...
func _on_room_ready() -> void:
	...
	if _role == 1 and _room_ready_count == 1:
		NetworkManager.rpc_authority_state.rpc({
...
func _on_server_snapshot(state: Dictionary) -> void:
	if _role != 2:
		return
	if int(state.get("state_revision", -1)) != 42:
